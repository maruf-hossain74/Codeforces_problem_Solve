...
int32_t main() {
    ...
    while(t--) {
        //cout << "Case " << ++cs << ": ";
        int n; cin>> n;
        vector<vector<int>> vs(n+2, vector<int>(0));
        vector<int> mp(n+1);
        int mx1 = 0;
        for(int i = 1; i < n; i++) {
            int x, y; cin>> x>> y;
        ...
        vector<int> mx;
        int mxx = *max_element(mp.begin(), mp.end());
        for(int i = 1; i <= n; i++) {
            if(mxx == mp[i]) {
                mx.push_back(i);
            }
        ...
        int p1 = 0, p1mx = -10;
        int sss = mx.size();
        for(int j = 0; j < min({int(10), sss}); j++) {
            int it = mx[j];
            auto dup = mp;
            for(auto &i: vs[it]) dup[i]--;
            dup[it] = -1;
            int nmx = *max_element(dup.begin(), dup.end());
            mx1 = max({mx1, nmx});
            dup.clear(); 
        }
        cout << mxx + mx1 - 1 << endl;
