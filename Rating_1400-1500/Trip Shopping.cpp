...
int32_t main() {
    ...
    while(t--) {
        ...
        int mx = inf;
        for(int i = 1; i < n; i++) {
            int a = vp1[i-1].first, b = vp1[i-1].second;
            int c = vp1[i].first, d = vp1[i].second;
            int cur = abs(a-b) + abs(c-d);
            int p1 = abs(a - c) + abs(b - d);
            int p2 = abs(a - d) + abs(c - b);
            int mxx = max({p1, p2});
            mx = min({mx, max({int(0), mxx - cur})});
        }
        for(int i = 1; i < n; i++) {
            int a = vp2[i-1].first, b = vp2[i-1].second;
            int c = vp2[i].first, d = vp2[i].second;
            int cur = abs(a-b) + abs(c-d);
            int p1 = abs(a - c) + abs(b - d);
            int p2 = abs(a - d) + abs(c - b);
            int mxx = max({p1, p2});
        ...
        int ans = 0;
        for(int i = 0; i < n; i++) {
            ans += abs(vp1[i].first - vp1[i].second);
        }
        cout << ans + mx << endl;
    }
    return 0;
