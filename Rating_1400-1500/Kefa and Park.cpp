...
int ans = 0;
void dfs(vector<vector<int>> &v, vector<int> &cat, vector<bool> &visited, int k, int node, int cnt) {
    visited[node] = true;
    if(cat[node] == 1) {
        cnt++;
        if(cnt > k) return;
