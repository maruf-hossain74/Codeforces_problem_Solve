/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
  ...
  $   Institute: JKKNIU                         $
      Problem: L_Chris_and_Magic_Square                
  $   Date: 2025/03/18                          $
  $   Time: 23:42:35                            $
  $                                             $
  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
...
int32_t main() {
    ...
    while(t--) {
        ...
        if(n==1) cout << 1 << endl;
            ...
            int sum = 0, need = 0, y = (x+1)%n;
            for(int i = 0; i < n; i++) sum += v[x][i];
            for(int i = 0; i < n; i++) need += v[y][i];
            v[x][k] = need-sum;
            if(v[x][k] < 1 || v[x][y] > 1e18) {
                cout << -1 << endl;
                continue;
            }
            bool ans = true;
            for(int i = 0; i < n && ans; i++) {
                int temp = 0;
                for(int j = 0; j < n && ans; j++) temp += v[i][j];
                if(temp != need) ans = false;
            }
            for(int i = 0; i < n && ans; i++) {
                int temp = 0;
                for(int j = 0; j < n && ans; j++) temp += v[j][i];
                if(temp != need) ans = false;
            }
            int temp = 0;
            for(int i = 0; i < n && ans; i++) temp += v[i][i];
            if(temp!=need) ans = false;
            temp = 0;
            for(int i = 0; i < n && ans; i++) temp += v[i][n-i-1];
            if(temp!=need) ans = false;
            if(ans) cout << need - sum << endl;
            else cout << -1 << endl;
        }
    }
    return 0;
