...
int solve(int n, int x, int y) {
    ...
    if(x == 1 && y == 1) ans++;
    else if(x == 1 && y == 2) ans += 4;
    else if(x == 2 && y == 1) ans += 3;
    else ans += 2;
    return ans;
}
vector<int> solve(int n, int x) {
    ...
    while(n>1) {
        ...
        int each = XY/4;
        if((x/each)==1) {
            if(x%each) ans[0] += (1<<(n-1)), ans[1] += (1 << (n-1));
        }
        else if((x/each)==2) {
            if(x%each) ans[0] += (1<<(n-1));
            else ans[0] += (1<<(n-1)), ans[1] += (1 << (n-1));
        }
        else if((x/each)==3) {
            if(x%each) ans[1] += (1<<(n-1));
            else ans[0] += (1<<(n-1));
        }
        else if((x/each)==4) ans[1] += (1<<(n-1));
        x -= ((x-1)/each)*each;
        n--;
    }
    if(x == 0 || x == 4) ans[1]++;
    else if(x == 2) ans[1]++, ans[0]++;
    else if(x == 3) ans[0]++;
    return ans;
}
int t = 1, cs = 0;
