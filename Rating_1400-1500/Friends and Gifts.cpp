...
int32_t main() {
    ...
    while(t--) {
        ...
        for(int i = 0; i < n; i++) {
            cin>> v[i];
            if(!v[i]) st.push_back(i);
        }
        set<int> fnd(v.begin(), v.end());
        for(int i = 1; i <= n; i++) {
            if(fnd.find(i) == fnd.end()) need.insert(i);
        }
        vector<int> take(need.begin(), need.end());
        for(auto &it: st) {
            if(take.size() == 2) {
                if(it+1 != take[1] && st.back()+1 != take[0]) v[it] = take[1], v[st.back()] = take[0];
                else v[it] = take[0], v[st.back()] = take[1];
                break;
            }
            else if(it+1 != take.back()) {
                v[it] = take.back();
                take.pop_back();
            }
            else {
