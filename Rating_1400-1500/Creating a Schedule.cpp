...
int32_t main() {
    ...
    while(t--) {
        ...
        for(int i = 0, j = m-1, k = 0; i < n; i++, j--, k++) {
            ans[i][0] = v[k], ans[i][1] = v[j], ans[i][2] = v[k];
            ans[i][3] = v[j], ans[i][4] = v[k], ans[i][5] = v[j];
            if(i < n-1) {
                i++;
                ans[i][0] = v[j], ans[i][1] = v[k], ans[i][2] = v[j];
                ans[i][3] = v[k], ans[i][4] = v[j], ans[i][5] = v[k];
            }
        }
        for(auto &it: ans) {
            for(auto &i: it) {
