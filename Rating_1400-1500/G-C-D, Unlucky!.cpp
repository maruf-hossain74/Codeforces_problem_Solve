...
int32_t main() {
    ...
    while(t--) {
        ...
        vector<int> v(n), k(n);
        int gcv = 0, gck = 0;
        for(auto &it: v) { cin>> it; gcv = __gcd(gcv, it); }
        for(auto &it: k) { cin>> it; gck = __gcd(gck, it); }
        //cout << gcv << " " << gck << endl;
        if(gcv != gck) cout << "NO" << endl;
        else {
            vector<int> arr(n);
            for(int i = 0; i < n; i++) arr[i] = (v[i]*k[i]) / __gcd(v[i], k[i]);
            bool ans = true;
            int gc = 0;
            for(int i = 0; i < n; i++) {
                gc = __gcd(gc, arr[i]);
