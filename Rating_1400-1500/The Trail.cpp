...
int32_t main() {
    ...
    while(t--) {
        ...
        int sum = 0, x = 0, y = 0;
        if(s[0] == 'D') for(int i = 0; i < m; i++) sum += v[0][i];
        else for(int i = 0; i < n; i++) sum += v[i][0];
        if(n != m) v[0][0] = -sum, sum = 0;
        for(int i = 0; i < s.size(); i++) {
            if(s[i] == 'D' && (i == s.size()-1 || s[i+1] == 'D')) {
                x++;
                int temp = 0;
                for(int a = 0; a < m; a++) temp += v[x][a];
                v[x][y] = sum - temp;
            }
            else if(s[i] == 'D') {
                x++;
                int temp = 0;
                for(int a = 0; a < n; a++) temp += v[a][y];
                v[x][y] = sum - temp;
            }
            else if(s[i] == 'R' && (i == s.size()-1 || s[i+1] == 'R')) {
                y++;
                int temp = 0;
                for(int a = 0; a < n; a++) temp += v[a][y];
                v[x][y] = sum - temp;
