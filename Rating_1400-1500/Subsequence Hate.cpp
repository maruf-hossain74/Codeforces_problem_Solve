...
int32_t main() {
    ...
    while(t--) {
        ...
        for(int i = 1; i < n; i++) {
            if(s[i] == '0' && s[i-1] == '1') oz = true;
            else if(s[i-1] == '0' && s[i] == '1') zo = true;
        }
        if(zo && oz) {
            int ans = 0;
            for(auto &it: s) it == '0'?ans++:0;
            ans = min({ans, n-ans});
            vector<int> v;
            int cnt = 1;
            for(int i = 1; i < n; i++) {
            ...
            //print(v);
            vector<int> even, odd;
            even.push_back(0); even.push_back(v[1]);
            odd.push_back(v[0]); odd.push_back(v[0]);
            for(int i = 2; i < v.size(); i++) {
                if(i%2){
                    even.push_back(v[i] + even.back());
                    odd.push_back(odd.back());
                }
                else {
                    odd.push_back(v[i] + odd.back());
                    even.push_back(even.back());
                }
            }
            //print(even); print(odd);
            for(int i = 1; i < v.size()-1; i++) {
                ans = min({odd[i-1]+even.back()-even[i], ans});
                ans = min({even[i-1]+odd.back()-odd[i], ans});
            }
            cout << ans << endl;
        }
        else cout << 0 << endl;
