...
vector<int> color;
bool ans;
void color_it(int x, vector<vector<int>> &v) {
    if(!visited[x]) {
        visited[x] = true;
        for(auto &it: v[x]) {
            if(color[it] == color[x]) {
...
int32_t main() {
    ...
    for(int T = 1; T <= t; T++) {
        ...
        for(int i = 0; i < n; i++) {
            ...
            mp[x]++, mp[y]++;
            //vp.push_back({x, y});
            if(mp[x]>2 || mp[y]>2) ans = false;
        }
        //for(auto &it: v) { for(auto &i: it) cout << i << " "; cout << endl; }
        for(int i = 1; i <= n && ans; i++)
            if(!visited[i]) color[i] = 0, color_it(i, v);
        // if(T == 1e12) {
        //     string find = to_string(n);
        //     for(auto &it: vp) find += to_string(it.first) + to_string(it.second);
        //     cout << find << endl;
        //     break;
        // }
