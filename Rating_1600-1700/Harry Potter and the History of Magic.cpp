/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
  ...
  $   Institute: JKKNIU                         $
      Problem: B_Harry_Potter_and_the_History_of_Magic                
  $   Date: 2025/03/17                          $
  $   Time: 00:02:32                            $
  $                                             $
  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
...
int solve(string s, int n) {
    ...
    for(int i = n; i < m; i++) {
        ...
        int cnt = 0;
        for(int j = 0; j < 4; j++) if(s[j]!=k[j]) cnt++;
        if(cnt==1) {
            return i;
        }
    }
    if(m>=n) return m;
    for(int i = m+1; i <= 2011; i++) {
        string k = to_string(i);
        int cnt = 0;
        for(int j = 0; j < 4; j++) if(s[j]!=k[j]) cnt++;
        if(cnt==1 && i >= n) {
...
int32_t main() {
    ...
    while(t--) {
        ...
        for(int i = 0; i < n; i++) {
            string s; cin>> s;
            if(i==0) v.push_back(solve(s,1000));
            else v.push_back(solve(s,v.back()));
        }
        if(v.back()<=2011) for(auto &it: v) cout << it << endl;
