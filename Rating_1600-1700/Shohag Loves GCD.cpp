...
int32_t main() {
    ...
    while(t--) {
        //cout << "Case " << ++cs << ": ";
        int n, k; cin>> n>> k;
        vector<int> v(k); for(auto &it: v) cin>> it;
        if(n >= power(2,k)) {
            cout << -1 << endl;
            continue;
        }
        ...
        for(int i = 1; i <= n; i++) {
            ...
            int ptr = lower_bound(v.begin(), v.end(), ans[i])-v.begin();
            for(int j = 2; i*j <= n; j++) {
                if(ans[i*j] != 0) ans[i*j] = min({ans[i*j], v[ptr-1]});
                else ans[i*j] = v[ptr-1];
            }
        }
