...
int32_t main() {
    ...
    while(t--) {
        ...
        for(int i = 0; i < n; i++) {
            cin>> v[i];
            if(v[i] == 0) zero.insert(i+1);
            else if(v[i] == 1) one.insert(i+1);
            else two.insert(i+1);
        }
        vector<pair<int,int>> ans;
        if(!one.empty() && !zero.empty() && !two.empty()) {
            while(*one.begin() < *zero.rbegin() || *one.rbegin() > *two.begin()) {
                int fst1 = *one.begin();
                int z0 = *zero.rbegin();
                if(fst1 < z0) {
                    ans.push_back({fst1, z0});
                    one.erase(fst1); zero.erase(z0);
                    one.insert(z0); zero.insert(fst1);
                }
                int lst1 = *one.rbegin();
                int t2 = *two.begin();
                if(lst1 > t2) {
                    ans.push_back({lst1, t2});
                    one.erase(lst1); two.erase(t2);
                    one.insert(t2); two.insert(lst1);
                }
            }
        }
        else if(zero.empty()) {
