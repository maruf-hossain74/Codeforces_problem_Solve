...
int32_t main() {
    ...
    //cin >> t;
    while(t--) {
        double m, v, d, t; cin>> m>> v>> d>> t;
        double misha = max({(3*m)/10.0, m - (m/250.00) * d});
        double vasia = max({(3*v)/10.0, v - (v/250.00) * t});
        if(misha == vasia) cout << "Tie" << endl;
        else if(misha > vasia) cout << "Misha" << endl;
