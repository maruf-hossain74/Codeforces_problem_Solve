...
int32_t main() {
    ...
    while(t--) {
        ...
        int kk = k;
        string s; cin>> s;
        string ans = "", ch = "abcdefghijklmnopqrstuvwxyz";
        map<char,int>mp;
        ...
        while(k--) {
            ...
            bool take = true;
            int sss = ans.size();
            for(int i = 0; i < ch.size() && sz>0; i++) {
                if(mp[ch[i]] > 0) { mp[ch[i]]--; sz--; }
                else if(take) { ans += ch[i]; take = false; }
            }
            if(sss == ans.size()) ans += ch[n/kk];
        }
        cout << ans << endl;
    }
    return 0;
