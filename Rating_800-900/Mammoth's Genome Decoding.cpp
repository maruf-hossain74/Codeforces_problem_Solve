/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
  ...
  $   Institute: JKKNIU                         $
      Problem: V_Mammoth_s_Genome_Decoding                
  $   Date: 2024/10/09                          $
  $   Time: 18:38:33                            $
  $                                             $
  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
...
int32_t main() {
    ...
    while(t--) {
        ...
        int rem = n - (mx*4);
        rem /= 4;
        A = mx-A+rem, T = mx-T+rem, G = mx-G+rem, C = mx-C+rem;
        for(int i = 0; i < n && A>0; i++) {
            if(s[i] == '?' ) {s[i] = 'A'; A--; } if(A == 0) break; }
        for(int i = 0; i < n && T>0; i++) {
            if(s[i] == '?' ) {s[i] = 'T'; T--; } if(T == 0) break; }
        for(int i = 0; i < n && G>0; i++) {
            if(s[i] == '?' ) {s[i] = 'G'; G--; } if(G == 0) break; }
        for(int i = 0; i < n && C>0; i++) {
            if(s[i] == '?' ) {s[i] = 'C'; C--; } if(C == 0) break; }
        cout << s << endl;
    }
    return 0;
}
