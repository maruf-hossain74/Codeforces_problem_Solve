/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
  ...
  $   Institute: JKKNIU                         $
      Problem: CJ_Non_Substring_Subsequence                
  $   Date: 2024/10/12                          $
  $   Time: 13:29:27                            $
  $                                             $
  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
...
int32_t main() {
    ...
    while(t--) {
        ...
        zero.push_back(0);
        for(auto &it: s) {
            if(it == '1') one.push_back(one.back() + 1), zero.push_back(zero.back());
            else zero.push_back(zero.back()+1), one.push_back(one.back());
        }

        while(q--) {
            bool ans = false;
            int x, y; cin>> x>> y;
            char lst = s[y-1], fst = s[x-1];
            if(fst == '0') {
                if(zero[x-1]) ans = true;
            }
            else {
                if(one[x-1]) ans = true;
            }
            if(lst == '0') {
                if(zero.back()-zero[y]) ans = true;
            }
            else {
                if(one.back()-one[y]) ans = true;
            }
