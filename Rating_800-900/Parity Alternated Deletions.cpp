/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
  ...
  $   Institute: JKKNIU                         $
      Problem: BQ_Parity_Alternated_Deletions                
  $   Date: 2024/10/11                          $
  $   Time: 15:58:33                            $
  $                                             $
  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
...
int32_t main() {
    ...
    while(t--) {
        ...
        int del = min({even.size(), odd.size()});
        if(del*2 == n) cout << 0 << endl;
        else {
            sort(even.begin(), even.end());
            sort(odd.begin(), odd.end());
            for(int i = 0; i < even.size()-del; i++) ans += even[i];
            for(int i = 0; i < odd.size()-del; i++) ans += odd[i];
            if(even.size()> odd.size()) ans -= even[even.size()-del-1];
            else ans -= odd[odd.size()-del-1];
            cout << ans << endl;
        }
