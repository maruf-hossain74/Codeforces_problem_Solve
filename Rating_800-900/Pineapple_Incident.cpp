...
int32_t main() {
    ...
    //cin >> t;
    while(t--) {
        int st, in, ne; cin>> st>> in>> ne;
        if(in == 2 && st != 0 && st <= ne && st != ne-1) cout << "YES" << endl;
        else if(st > ne) cout << "NO" << endl;
        else {
            if(st == ne-1) { cout << "NO" << endl; continue; }
            ne -= st;
            if(ne % in == 0 || ne % in == 1) cout << "YES" << endl;
            else cout << "NO" << endl;
        }
    }
    return 0;
