/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
  ...
  $   Institute: JKKNIU                         $
      Problem: C_Cirno_and_Operations                
  $   Date: 2025/01/26                          $
  $   Time: 21:03:36                            $
  $                                             $
  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
...
int32_t main() {
    ...
    while(t--) {
        ...
        while(v.size() > 1) {
            ...
            for(int i = 0; i < new1.size()-1; i++) new1[i] = new1[i+1]-new1[i];
            new1.pop_back();
            for(int i = 0; i < rev.size()-1; i++) rev[i] = rev[i+1]-rev[i];
            rev.pop_back();
            for(auto &it: rev) sum2 += it;
            for(auto &it: new1) sum1 += it;
            if(sum1 > sum2) v = new1;
            else v = rev;
            ans = max({ans, sum});
            ans = max({ans, sum1});
