/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
  ...
  $   Institute: JKKNIU                         $
      Problem: B_Gellyfish_and_Baby_s_Breath                
  $   Date: 2025/05/31                          $
  $   Time: 21:02:08                            $
  $                                             $
  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
...
int power(int num, int pow) { int res = 1; while (pow) { if (pow%2==1)
    res = (res * num) % md; num = (num * num) % md; pow/=2; } return (res%md); }
...
int32_t main() {
    ...
    while(t--) {
        ...
        int mxv = -1, mxk = -1, inv = 0, ink = 0;
        vector<int> ans(n);
        for(int i = 0; i < n; i++) {
            if(i == 0) mxv = v[i];
            else if(mxv < v[i]) mxv = v[i], inv = i;
            if(i == 0) mxk = k[i];
            else if(mxk < k[i]) mxk = k[i], ink = i;
            int alv = i-inv, alk = i-ink;
            if(mxv > mxk) ans[i] = (power(2, mxv) + power(2, k[alv])) % md;
            else if(mxv < mxk) ans[i] = (power(2, mxk) + power(2, v[alk])) % md;
            else {
                int pfv = mxv + k[alv];
                int pfk = mxk + v[alk];
                if(pfv > pfk) ans[i] = (power(2, mxv) + power(2, k[alv])) % md;
                else ans[i] = (power(2, mxk) + power(2, v[alk])) % md;
            }
        }
