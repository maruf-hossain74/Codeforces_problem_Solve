/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
  ...
  $   Institute: JKKNIU                         $
      Problem: B_Replace_and_Keep_Sorted                
  $   Date: 2024/09/25                          $
  $   Time: 14:06:19                            $
  $                                             $
  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
...
int32_t main() {
    ...
    while(t--) {
        int n, q, k; cin>> n>> q>> k;
        vector<int> v(n,0),left(n,0),right(n,0),mid(n,0);
        for(auto &it: v) cin>> it;
        for(int i = 0; i < n-1; i++) left[i] = v[i+1] - 2;
        for(int i = 1; i < n; i++) right[i] = k - v[i-1] - 1;
        for(int i = 1; i < n-1; i++) mid[i] = v[i+1] - v[i-1] - 2;
        for(int i = 0; i < n; i++) if(i > 0) mid[i] += mid[i-1];
        while(q--) {
            int x, y; cin>> x>> y;
            if(x == y) cout << k-1 << endl;
            else if(x == y-1) cout << left[x-1] + right[y-1] << endl;
            else cout << left[x-1] + right[y-1] + mid[y-2] - mid[x-1] << endl;
        }
    }
    return 0;
}
