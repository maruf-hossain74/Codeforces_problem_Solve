...
int32_t main() {
    ...
    while(t--) {
        ...
        while(v.front() != k) {
            for(int i = 1; i < n; i++) {
                if(v[i-1] != v[i]) v[i-1] = min({k, v[i-1]+1});
                if(i == n-1) v[i] = min({k, v[i]+1});
            }
            sort(v.begin(), v.end());
            ans++;
        }
        ...
        // }
        // // sort(v.begin(), v.end());
        // // if(f) cout << v.size() << endl;
        // // else cout << k - v.back() + v.size() - 1 << endl;
        // int ans = 0;
        // for(auto &it: mp) {
        //     // int res = 0, f = k-it.first;
        //     // for(int i = it.second; i>0; i--) {
        //     //     res += min({int(1), f--});
        //     // }
        //     ans = max({k-it.first+it.second-1, ans});
        // }
        // cout << ans << endl;
