...
int32_t main() {
    ...
    while(t--) {
        ...
        vector<int> real_pre(n), real_post(n), full_pre(n);
        k[0] == 1 ? real_pre[0] = v[0]: real_pre[0] = 0;
        k[n-1] == 1 ? real_post[n-1] = v[n-1]: real_post[n-1] = 0;
        full_pre[0] = v[0];
        for(int i = 1; i < n; i++) full_pre[i] = full_pre[i-1]+v[i], k[i]==1?real_pre[i] = real_pre[i-1]+v[i]:real_pre[i]=real_pre[i-1];
        for(int i = n-2; i >= 0; i--) k[i]==1?real_post[i]=real_post[i+1]+v[i]:real_post[i]=real_post[i+1];
        //print(full_pre); print(real_pre); print(real_post);
        int ans = 0;
        for(int i = 0; i <= n-m; i++) {
            //cout << i << " ";
            int l = i-1, r = i+m;
            int mid = full_pre[r-1];
            if(l >= 0) mid -= full_pre[l];
            int left, right;
            (l >= 0) ? left = real_pre[l]: left = 0;
            r < n? right = real_post[r]: right = 0;
