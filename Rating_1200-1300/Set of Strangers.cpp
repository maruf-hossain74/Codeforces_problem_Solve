...
int32_t main() {
    ...
    while(t--) {
        ...
        set<int> st, cnt;
        for(auto &it: v) for(auto &i: it) { cin>> i; st.insert(i); }
        int ans = 0;
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < m; j++) {
                if(j > 0 && v[i][j]==v[i][j-1]) cnt.insert(v[i][j]);
                if(j < m-1 && v[i][j]==v[i][j+1]) cnt.insert(v[i][j]);
                if(i > 0 && v[i][j]==v[i-1][j]) cnt.insert(v[i][j]);
                if(i < n-1 && v[i][j]==v[i+1][j]) cnt.insert(v[i][j]);
            }
        }
        if(cnt.empty()) cout << st.size() - 1 << endl;
        else cout << st.size()-cnt.size() + 2*(cnt.size()-1) << endl;
    }
    return 0;
}
