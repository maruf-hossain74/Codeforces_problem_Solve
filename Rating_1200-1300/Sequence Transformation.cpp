...
int32_t main(){
    ...
    while(t--) {
        ...
        cin>>n;
        int A[n];
        bool checker = false,finish = false;
        map<int,int>mp;
        for(int i = 0; i < n; i++) {
            cin>>A[i];
            if(i!=0) {
                if(A[i] != A[i-1] && mp.empty()) {
                    mp[A[i-1]]++;
                    checker = true;
                }
                else if(A[i] != A[i-1] && mp[A[i-1]] == 0) mp[A[i-1]] += 2;
                else if(A[i] != A[i-1]) mp[A[i-1]]++;
                if(i == n-1 && mp[A[i]] == 0 && checker) finish = true;
            }
        }
