...
int32_t main() {
    ...
    while(t--) {
        ...
        for(int i = 1; i < m; i++) sup.push_back(v[i]-v[i-1]);
        bool ans = true;
        for(int i = 1; i < sup.size(); i++) if(sup[i] < sup[i-1]) ans = false;
        if(!ans) cout << "No" << endl;
        else {
            int fst = v[0]/rest + (v[0]%rest > 0);
            if(fst > sup.front()) ans = false;
