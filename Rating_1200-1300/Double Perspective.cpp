...
int32_t main() {
    ...
    while(t--) {
        ...
        int x = 1;
        for(auto &it: v) { cin>> it.first>> it.second.first; it.second.second = x; x++; }
        sort(v.begin(), v.end());
        ...
        //     }
        //     else {
        //         if(vp.back().second >= it.first) {
        //             vp.back().second = max({it.second, vp.back().second});
        //         }
        //         else {
        //             vp.push_back({it.first, it.second});
        //         }
        ...
        for(int i = 1; i < n; i++) {
            if(v[i-1].first != v[i].first) {
                ans.push_back(v[i-1].second.second);
            }
            if(i == n-1) ans.push_back(v[i].second.second);
        }
        cout << ans.size() << endl;
        print(ans);
    }
