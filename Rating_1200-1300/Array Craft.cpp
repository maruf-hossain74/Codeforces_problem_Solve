...
int32_t main() {
    ...
    while(t--) {
        ...
        if(x <= y) {
            ...
            for(int i = x-1, j = 0; j < mn; j++, i--) v[i] = 1;
            for(int i = y-1, j = 0; j < mn; j++, i++) v[i] = 1;
            for(auto &it: v) if(it==0) it =-1;
        }
        else {
            for(int i = y-1; i < x; i++) v[i] = 1;
            if((y-1)%2) for(int i = 0; i < y-1; i++) v[i] = i%2?1:-1;
            else for(int i = 0; i < y-1; i++) v[i] = i%2?-1:1;
            //for(auto &it: v) cout << it <<" "; cout << endl;
            if((n-x)%2) for(int i = n-1, j = 0; i >= x; i--, j++) v[i]=j%2?1:-1;
            else for(int i = n-1, j = 0; i >= x; i--, j++) v[i]=j%2?-1:1;
        }
        for(auto &it: v) cout << it <<" "; cout << endl;
    }
    return 0;
