/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
  ...
  $   Institute: JKKNIU                         $
      Problem: C_2_Skibidus_and_Fanum_Tax_hard_version                
  $   Date: 2025/03/10                          $
  $   Time: 23:16:12                            $
  $                                             $
  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
    ...
    for(int T = 1; T <= t; T++) {
        ...
        for(int i = 0; i < n; i++) {
            if(i == 0) {
                ...
                //cout << m << " " << mn << endl;
                if(m != mn) {
                    int f1 = min({v[i], s[mn]-v[i]});
                    int f2 = max({v[i], s[mn]-v[i]});
                    if(f1 >= v[i-1]) v[i] = f1;
                    else if(v[i] >= v[i-1]) continue;
                    else if(f2 >= v[i-1]) v[i] = f2;
                }
            }
        }
        //for(auto &it: v) cout << it << " "; cout << endl;
        bool ans = true;
        for(int i = 1; i < n; i++) if(v[i-1] > v[i]) ans = false;
