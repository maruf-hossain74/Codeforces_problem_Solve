...
int32_t main() {
    ...
    while(t--) {
        ...
        for(int i = 0; i < n; i++) {
            ...
            string str = dtob(it);
            int x = str.size()-1, cnt1 = 0;
            while(x >= 0 && str[x] == '0') x--, cnt1++;

            string st2 = dtob(it * (i+1));

            int y = st2.size()-1, cnt2 = 0;
            while(y >= 0 && st2[y] == '0') y--, cnt2++;

            get.push_back(cnt2 - cnt1);
            cnt+=cnt1;
        }
        int ans = 0;
