...
string dtob(int n){ string m = ""; while(n!=0){ if(n%2==0) m='0'+m; else m='1'+m; n/=2; }
    while(m.size()!=32) m = "0"+m; while(m.size() != 32) m = "0"+m; return m;}
...
int32_t main() {
    ...
    while(t--) {
        ...
        for(auto &it: v) {
            cin>> it;
            string s = dtob(it);
            for(int i = 0; i < s.size(); i++) s[i]=='1'?bit[32-i]++:0;
        }
        int ans = 0;
        for(auto &it: v) {
            string s = dtob(it);
