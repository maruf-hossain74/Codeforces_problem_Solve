/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
  ...
  $   Institute: JKKNIU                         $
      Problem: B_Outstanding_Impressionist                
  $   Date: 2024/12/28                          $
  $   Time: 21:12:24                            $
  $                                             $
  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
...
int32_t main() {
    ...
    while(t--) {
        ...
        for(int i = 0; i < n; i++) {
            ...
            mp[{x,y}]++;
            vp.push_back({x, y});
            if(x == y && (st.empty() || st.find(x) == st.end())) v.push_back(x);
            if(x == y) st.insert(x);
        }
        sort(v.begin(), v.end());
        string ans = "";
        for(int i = 0; i < n; i++) {
            if(vp[i].first == vp[i].second && mp[{vp[i].first, vp[i].second}] > 1) ans += "0"; 
            else if(vp[i].first == vp[i].second && mp[{vp[i].first, vp[i].second}] == 1) ans += "1"; 
            else {
                int lb = lower_bound(v.begin(), v.end(), vp[i].first) - v.begin();
                int ub = upper_bound(v.begin(), v.end(), vp[i].second) - v.begin();
                if(ub - lb == vp[i].second - vp[i].first + 1) ans += "0";
                else ans += "1";
            }
        }
        cout << ans << endl;
