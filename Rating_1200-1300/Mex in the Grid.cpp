...
int32_t main() {
    ...
    while(t--) {
        ...
        for(int i = 0; i < n/2; i++) {
            for(int a = i; a < n-i; a++) v[i][a] = mx--;
            for(int b = i+1; b < n-i; b++) v[b][n-i-1] = mx--;
            for(int c = n-i-2; c >= i; c--) v[n-i-1][c] = mx--;
            for(int d = n-i-2; d > i; d--) v[d][i] = mx--;
        }
        for(auto &it: v) {
