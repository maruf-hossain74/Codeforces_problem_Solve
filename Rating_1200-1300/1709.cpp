...
bool check(vector<int> &v, vector<int> &k) {
    int n = v.size();
    for(int i = 0; i < n; i++) {
        if(i < (n-1)) {
            if(v[i] > v[i+1] || k[i] > k[i+1] || v[i] > k[i]) {
                //cout << "false 1" << endl; 
                return false;
            }
        }
...
void upd(vector<int> &v, vector<int> &k, vector<pair<int, int>> &vp) {
    int n = v.size();
    for(int i = 0; i < n; i++) {
        if(v[i] > k[i]) {
            vp.push_back({3, i+1});
...
void soort(vector<int> &v, vector<int> &k, vector<pair<int, int>> &vp) {
    int n = v.size();
    for(int i = 0; i < n-1; i++) {
        for(int j = 0; j < n-1; j++) {
            if(v[j] > v[j+1]) {
