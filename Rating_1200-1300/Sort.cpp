...
int32_t main() {
    ...
    while(t--) {
        ...
        int n, q; cin>> n>> q;
        string s, k; cin>> s>> k;
        vector<vector<int>> ss(n+1, vector<int> (26)), kk(n+1, vector<int> (26));
        for(int i = 1; i <= n; i++) {
            for(int j = 0; j < 26; j++) {
                ss[i][j] = ss[i-1][j];
                kk[i][j] = kk[i-1][j];
        ...
        // for(auto &it: ss) {
        //     for(auto &i: it) cout << i << " ";cout << endl; cout << endl;
        // }
        // for(auto &it: kk) {
        //     for(auto &i: it) cout << i << " ";cout << endl; cout << endl;
        // }
        while(q--) {
            ...
            vector<int> v1, v2;
            for(int i = 0; i <26; i++) {
                if(x == 1) v1.push_back(ss[y][i]);
                else v1.push_back(ss[y][i] - ss[x-1][i]);
            }
            for(int i = 0; i <26; i++) {
                if(x == 1) v2.push_back(kk[y][i]);
                else v2.push_back(kk[y][i] - kk[x-1][i]);
            }
