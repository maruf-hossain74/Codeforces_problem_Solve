...
int32_t main() {
    ...
    while(t--) {
        ...
        vector<int> v(n);
        bool ans = false;
        for(auto &it: v) { cin>> it; if(it == 0) ans = true; } 
        if(!ans) cout << 1 << endl << 1 << " " << n << endl;
        else {
            if(v.back() != 0) cout << 2 << endl << 1 << " " << n-1 << endl << 1 << " " << 2 << endl;
            else if(v.front() != 0) cout << 2 << endl << 2 << " " << n << endl << 1 << " " << 2 << endl;
            else cout << 3 << endl << 1 << " " << 2 << endl << 2 << " " << n-1 << endl << 1 << " " << 2 << endl;
        }
    }
    return 0;
}
