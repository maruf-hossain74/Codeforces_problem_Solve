/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
  ...
  $   Institute: JKKNIU                         $
      Problem: B_Turtle_and_an_Infinite_Sequence                
  $   Date: 2025/03/23                          $
  $   Time: 23:56:38                            $
  $                                             $
  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
...
int power(int num, int pow) { int res = 1; while (pow) { if (pow & 1)
    res = (res * num); num = (num * num); pow >>= 1; } return res;}
...
int32_t main() {
    ...
    while(t--) {
        //cout << "Case " << ++cs << ": ";
        int n, m; cin>> n>> m;
        int x = max({int(0), n-m}), y = n+m;
        string s1 = dtob(x), s2 = dtob(y);
        if(s2.size() != s1.size()) cout << power(2, s2.size()) - 1 << endl;
        else {
            string s = "";
            bool ans = true;
            for(int i = 0; i < s2.size(); i++) {
                if(s2[i] == s1[i] && ans) s += s1[i];
                else if(s2[i] != s1[i] && ans) {
                    s += "1";
                    ans = false;
                }
                else s += "1";
