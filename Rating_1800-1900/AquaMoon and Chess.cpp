...
vector<int> fact(1000001, 0), factinv(1000001, 0);
int md = 998244353;
...
int genfact(int n) {
    if(!n) return 1;
    return fact[n] ? fact[n] : fact[n] = n * genfact(n - 1) % md;
}

int genfactinv(int n) {
    if(!n) return 1;
    return factinv[n] ? factinv[n] : factinv[n] = power(genfact(n), md - 2);
}

int ncrmod(int n, int r) {
    return (genfact(n) * genfactinv(r) % md) * genfactinv(n - r) % md;
}
...
int32_t main() {
    ...
    while(t--) {
        ...
        for(int i = 0; i < n; i++) {
            if(s[i] == '0') zero++;
            else if(i+1 < n && s[i+1] == '1') one++,i++;
        }
        int ans = ncrmod(one+zero, one) % md;
        cout << ans << endl;
    }
    return 0;
}
