...
#define wow int n; cin>> n; vector<int> v(n); for(auto &it: v) cin>> it
...
int rem_gcd(vector<int> &v, int x) {
    int gc = 0;
    for(int i = 0; i < v.size(); i++) {
        if(x == i) continue;
        if(gc == 0) gc = v[i];
        else gc = __gcd(gc, v[i]);
    }
    return gc;
}
int t = 1, cs = 0;
int32_t main() {
    ...
    while(t--) {
        ...
        for(int i = 0; i < n; i++) {
            if(i == 0) gc = v[i];
            else gc = __gcd(v[i], gc);
        }
        if(gc == 1) {
            cout << 0 << endl;
            continue;
        }
        if(__gcd(__gcd(n, v[n-1]),rem_gcd(v, n-1)) == 1) cout << 1 << endl;
        else if(__gcd(__gcd(n-1, v[n-2]),rem_gcd(v, n-2)) == 1) cout << 2 << endl;
        else cout << 3 << endl;
    }
    return 0;
}
