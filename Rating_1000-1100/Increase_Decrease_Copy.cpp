/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
  ...
  $   Institute: JKKNIU                         $
      Problem: B_Increase_Decrease_Copy                
  $   Date: 2025/04/04                          $
  $   Time: 22:54:13                            $
  $                                             $
  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
...
int32_t main() {
    ...
    while(t--) {
        ...
        int ans = 1;
        int mn2 = inf, mx2 = -inf, mx1 = -inf, mn1 = inf, fin = inf;
        bool flag = false;
        for(int i = 0; i < n; i++) {
            ans += abs(v1[i]-v2[i]);
            mn2 = min({mn2, v2[i]});
            ...
            mn1 = min({mn1, v1[i]});
            mx1 = max({mx1, v1[i]});
            fin = min({fin, abs(v1[i]-v2[n])});
            fin = min({fin, abs(v2[i]-v2[n])});
            int mn = min({v1[i], v2[i]});
            int mx = max({v1[i], v2[i]});
            if(mn <= v2[n] && v2[n] <= mx) flag = true;
        }
        ...
        int cd = min({c, d});
        int temp = min({ab, cd});
        if(!flag) ans += min({temp, fin});
        //cout << temp << " " << flag << " ";
        //cout << min({min({abs(mn2-v2[n]), abs(mx2-v2[n])}), min({abs(mn1-v2[n]), abs(mx1-v2[n])})}) << endl;
        cout << ans << endl;
    }
    return 0;
}
