...
int32_t main() {
    ...
    while(t--) {
        ...
        for(auto &it: v) {
            cin>> it;
            sum += it;
            if(pre.empty()) pre.push_back(it);
            else pre.push_back(pre.back()+it);
        }
        vector<int> l(n), r(n, inf);
        for(int i = 0; i < n; i++) i==0?l[i]=v[i]:l[i] = max({l[i-1], v[i]});
        for(int i = n-1; i >= 0; i--) i==(n-1)?r[i]=v[i]:r[i] = min({r[i+1], v[i]});
        for(int i = n-1; i >= 0; i--) {
            if(i == 0) cout << sum << endl;
            else cout << sum - pre[i-1] + max({int(0), l[i-1]-v[i]}) << " ";
        }
    }
    return 0;
}
