/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
  ...
  $   Institute: JKKNIU                         $
      Problem: B_Asterisk_Minor_Template                
  $   Date: 2024/12/26                          $
  $   Time: 12:09:26                            $
  $                                             $
  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
...
int32_t main() {
    ...
    while(t--) {
        ...
        for(int i = 1; i < s.size(); i++) st1.insert({s[i-1], s[i]});
        for(int i = 1; i < k.size(); i++) st2.insert({k[i-1], k[i]});
        char a = '*', b = '*';
        for(auto &it: st1) {
        ...
        if(a == '*' && b == '*') {
            if(s[0] == k[0]) cout << "YES\n" << s[0] << "*" << endl;
            else if(s[s.size()-1] == k[k.size()-1]) cout << "YES\n" << "*" << s[s.size()-1] << endl;
            else cout << "NO" << endl;
        }
        else cout << "YES\n" << "*" << a << b << "*" << endl;
    }
