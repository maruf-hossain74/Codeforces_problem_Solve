...
int32_t main() {
    ...
    while(t--) {
        ...
        int n; cin>> n;
        int ans = 0, cnt = 0;
        // vector<pair<pair<int,int>, pair<int,int>>> vp(n);

        // // 1 top kechal
        // for(int i = 0; i < n; i++) {
        //     cin>> vp[i].first.first>> vp[i].first.second>> vp[i].second.first>> vp[i].second.second;
        //     int a = vp[i].first.first, b = vp[i].first.second, c = vp[i].second.first, d = vp[i].second.second;
        //     if(a == 0 && b > d) {
        //         ans += b - d;
        //         vp[i].first.second = d;
        //     }
        ...
        //     if(d > b) {
        //         int need = d-b;
        //         vp[i].first.second += min({cnt, need});
        //         cnt -= min({cnt, need});
        //     }
        //     //cout << vp[i].first.first << " " << vp[i].first.second << " " << vp[i].second.first << " " << vp[i].second.second << endl;
            
        //     if(cnt == 0) break;
        // }
        
        // //0 khali + refill korar kechal
        // cnt = 0;
        // for(int i = 0; i < n; i++) {
        //     int a = vp[i].first.first, b = vp[i].first.second, c = vp[i].second.first, d = vp[i].second.second;
        //     if(a > c && b <= d) {
        ...
        // //again 1 top korar kechal + final cal
        // for(int i = 0; i < n; i++) {
        //     int a = vp[i].first.first, b = vp[i].first.second, c = vp[i].second.first, d = vp[i].second.second;
        //     if(c == 0 && b > d) ans += min({2*a + b-d, 2*(b-d)});
        //     //cout << vp[i].first.first << " " << vp[i].first.second << " " << vp[i].second.first << " " << vp[i].second.second << endl;
        // }
