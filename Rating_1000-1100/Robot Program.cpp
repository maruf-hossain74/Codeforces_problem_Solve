...
int32_t main() {
    ...
    for(int T = 1; T <= t; T++) {
        //cout << "Case " << ++cs << ": ";
        string s; int n, x, k, ans = 0, cnt = 0; cin>> n>> x>> k>> s;
        for(int i=0; i<n && k>0 && x!=0; i++,k--) s[i]=='L'?x--:x++; x==0?ans++:0;
        if(!x)for(int i=0; i<n; i++,cnt++) s[i]=='L'?x--:x++,x==0?i=n:0;;
        !x&&cnt?(k&&x==0)?ans+=k/cnt:0:0;
        cout << ans << endl;
    }
    return 0;
}
