...
int dfs(int x, int y, int n, int m) {
    cnt += s[x][y];
    visited[x][y] = true;
    if(x+1 < n && s[x+1][y] != 0 && !visited[x+1][y]) {
        dfs(x+1, y, n, m);
    }
    if(x-1 >= 0 && s[x-1][y] != 0 && !visited[x-1][y]) {
        dfs(x-1, y, n, m);
    }
    if(y-1 >= 0 && s[x][y-1] != 0 && !visited[x][y-1]) {
        dfs(x, y-1, n, m);
    }
    if(y+1 < m && s[x][y+1] != 0 && !visited[x][y+1]) {
        dfs(x, y+1, n, m);
    }
    return cnt;
}
...
int32_t main() {
    ...
    while(t--) {
        ...
        visited.assign(n+1, vector<bool> (m, false));
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < m; j++) {
                cin >> s[i][j];
            }
        ...
        // for(int i = 0; i < n; i++) {
        //     for(int j = 0; j < m; j++) cout << s[i][j] << " "; cout << endl;
        // }
        cout << ans << endl;
    }
    return 0;
