...
int32_t main() {
    ...
    while(t--) {
        ...
        for(int i = 1; i*i <= n; i++) {
            ...
            if(n%i == 0) if(i*i != n) div.push_back(n/i);
        }
        sort(div.rbegin(), div.rend());
        //for(auto &it: div) cout << it << endl;
        for(auto &it: div) {
            string s = dtob(it);
            int x = s.size();
            if(!(x&1)) continue;
                ...
                bool find = true;
                for(int i = 0; i <= x/2; i++) if(s[i] != '1') find = false;
                for(int i = x/2 + 1; i < x; i++) if(s[i] != '0') find = false;
                if(find) {
                    cout << it << endl;
                    break;
                }
