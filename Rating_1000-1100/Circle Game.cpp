...
int32_t main() {
    ...
    while(t--) {
        ...
        int n; cin>> n;
        vector<int> v(n);
        int pm, pj, mm = 1e12, mj = 1e12;
        for(int i = 0; i < n; i++) {
            cin>> v[i];
            if(i%2) { if(mj > v[i]) { mj = v[i]; pj = i; }}
            else { if(mm > v[i]) { mm = v[i]; pm = i; }}
        }
        if(n%2) cout << "Mike" << endl;
        else {
            //cout << mm << " " << mj << " " << pm << " " << pj << endl;
            if(mm > mj) cout << "Mike" << endl;
            else if(mm < mj) cout << "Joe" << endl;
            else if(pm > pj) cout << "Mike" << endl;
            else cout << "Joe" << endl;
        }
    }
    return 0;
