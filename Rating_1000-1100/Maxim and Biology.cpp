...
int32_t main() {
    ...
    while(t--) {
        ...
        string k = "ACTG";
        for(int i = 0; i < n-3; i++) {
            int a = min({26-(abs(k[0]-s[i])), abs(k[0]-s[i])});
            int b = min({26-(abs(k[1]-s[i+1])), abs(k[1]-s[i+1])});
            int c = min({26-(abs(k[2]-s[i+2])), abs(k[2]-s[i+2])});
            int d = min({26-(abs(k[3]-s[i+3])), abs(k[3]-s[i+3])});
            ans = min({ans, a+b+c+d});
        }
        cout << ans << endl;
    }
