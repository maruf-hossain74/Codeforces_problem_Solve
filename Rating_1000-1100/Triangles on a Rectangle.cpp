...
int32_t main() {
    ...
    while(t--) {
        ...
        int n3; cin>> n3;
        int C[n3];
        for(int j = 0; j < n3; j++) cin>> C[j];
        
        int n4; cin>> n4;
        int D[n4];
        for(int j = 0; j < n4; j++) cin>> D[j];

        sort(A, A+n1);sort(B, B+n2); sort(C, C+n3); sort(D, D+n4);

        cout << max({k*(A[n1-1] - A[0]), k*(B[n2-1]-B[0]), n*(C[n3-1]-C[0]), n*(D[n4-1]-D[0])}) << endl;
