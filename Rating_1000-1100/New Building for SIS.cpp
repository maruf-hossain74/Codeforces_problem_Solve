/*$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
  ...
  $   Institute: JKKNIU                         $
      Problem: A_New_Building_for_SIS                
  $   Date: 2025/02/15                          $
  $   Time: 20:00:45                            $
  $                                             $
  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$*/
...
int32_t main() {
    ...
    while(t--) {
        //cout << "Case " << ++cs << ": ";
        int n, h, a, b, q; cin>> n>> h>> a>> b>> q;
        while(q--) {
            int t1, f1, t2, f2; cin>> t1>> f1>> t2>> f2;
            int ans = abs(t1-t2);
            if(t1 == t2) {
                ans += abs(f2-f1);
                cout << ans << endl;
            ...
            int set;
            if(a <= f1 && f1 <= b) set = f1;
            else set = (abs(a-f1)<=abs(b-f1)?a:b);
            //cout <<set << endl;
            if(!(a <= f1 && f1 <= b)) ans += min({abs(a-f1),abs(b-f1)});
            ans += abs(set-f2);
            cout << ans << endl;
        } 
    }
