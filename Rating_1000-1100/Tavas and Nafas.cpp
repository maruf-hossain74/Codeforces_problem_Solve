...
int32_t main() {
    ...
    while(t--) {
        ...
        vector<string> s3 = {"twenty","thirty","forty","fifty","sixty","seventy","eighty","ninety"};
        string s; cin>> s;
        if(s.size() == 1) cout << s1[s[0]-'0'] << endl;
        else if(s[0] == '1') cout << s2[s[1]-'0'] << endl;
        else if(s[1] == '0') cout << s3[s[0]-'0'-2] << endl;
        else cout << s3[s[0]-'0'-2] << "-" << s1[s[1]-'0'] << endl;
    }
    return 0;
}
