...
int32_t main() {
    ...
    while(t--) {
        ...
        for(int i = 0; i < n+1; i++) {
            for(int j = 0; j < n; j++) {
                cin>> v[i][j];
                if(j == 0) mp[v[i][j]].first++, mp[v[i][j]].second = i;
            }
        }
        int x = 0, y = 0;
        for(auto &it: mp) {
            if(it.second.first == 1) y = it.second.second;
            else x = it.second.second;
        }
        cout << v[x][0] << " ";
        for(auto &it: v[y]) cout << it << " "; cout << endl;
    }
    return 0;
}
