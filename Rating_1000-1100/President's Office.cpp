...
int32_t main() {
    ...
    while(t--) {
        ...
        for(auto &it: v) for(auto &i: it) cin>> i;
        set<char> ch;
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < m; j++) {
                if(i+1 < n && v[i][j]==c) ch.insert(v[i+1][j]);
                if(i-1 >=0 && v[i][j]==c) ch.insert(v[i-1][j]);
                if(j-1 >=0 && v[i][j]==c) ch.insert(v[i][j-1]);
                if(j+1 < m && v[i][j]==c) ch.insert(v[i][j+1]);
            }
        }
        //cout << ch.size() << endl;
        cout << ch.size() - (ch.find('.') != ch.end()) - (ch.find(c) != ch.end()) << endl;
